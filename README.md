...
Интерактивное меню выбора SOrg (3801–3806), Enter = значение из `config.yaml`.
...
Одна строка отчёта в общем случае = **клиент (SOrg. + Customer)**.  
Исключение: если у **Trade Name** в справочнике несколько кодов **TN_CH6**, клиент **размножается** на отдельные строки (по одной на каждый `6th level`).
...
| **CH6_Name** | 3805 Base (опционально) | SOrg. + Customer |
...
**TN_CH6:** каждый код из справочника — **отдельная строка** (не склейка через запятую).  
Пример: Trade Name = LENTA, в справочнике 4 кода → 4 строки с TN_CH6 `38110501` … `38110505` и именами LENTA HM / MM / SM / DC.
...
- с A8: если сработала логика At work/Education (см. Key)
...
| At work / Education | + **A8** между A7 и ZW_A7 |

**Условия для A8 и обратной проверки:**

| CH6_CGrp (повод At w/Ed) | CGrp (бизнес-тип) | Результат |
|--------------------------|-------------------|-----------|
| не P | любой | Key без A8 |
| P | P или Q | Key **с A8** |
| P | не P и не Q | Key **пустой** (ошибка данных) |

P/Q в CGrp сами по себе **не** включают A8 — нужен повод `CH6_CGrp = P`.
...
| **Check CH6 Customer & ZW** | CH6 = один из ZW_CH6; только при Grp4 = IN и заполненных CH6, ZW_CH6, ZW |
| **Check CH6 Customer & TN** | CH6 = TN_CH6 на этой строке; нужны CH6, TN_CH6, Trade Name |
...
| Параметр | Эффект |
|----------|--------|
| `dedupe: false` | Не схлопывать дубликаты по ключу — **размножать строки** (TN_CH6) |
| `aggregate` | Несколько значений на ключ → одна ячейка через `, ` (ZW, ZW_A7) |
...
3. Запуск: `python merge_columns.py -s 3805 --no-menu`
